...
New extracurricular applications use five weekly hours and are refused above the 60-hour limit without consuming an attempt. Jobs are checked using their offered hours, subtracting any replaced job. Clubs, sports and jobs gain two performance points per Age Up before workload penalties. At work includes Hours, Raise, Resign and Work harder. Requests depend on performance and years in position and occur once per year; training adds five performance once yearly. Approved hours never exceed 20 or total schedule 60. Positive manager Compliment/Conversation/Gift responses can increase work performance once per action/year. Job listings use relevant emojis and three reversible sorting buttons (name, wage, hours), plus Clear filters. Club/sport browse descriptions are removed.
