...
export function romanceOutcome(life:Life,person:Person,action:RelationshipAction){
 ...
 const chance=action==='Make love'?person.strength/100:Math.min(.95,.1+life.stats.Looks/250+person.strength/200)*(alreadyDating?.4:1);
 const accepted=compatible && random()<chance;
