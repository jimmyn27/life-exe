...
export function eventForAge(age: number): LifeEvent {
  const childhood = childhoodEvent(age);
  if (childhood) return childhood;
  if (age === 15) return {category:'Education',title:'Starting high school',text:'You have completed middle school and started high school. Graduation is three years away.',choices:[{label:'Focus on my future',hint:'Build good study habits.',outcome:'I started high school and focused on my future.',effect:{Smarts:3},schoolEffect:{grades:4}},{label:'Meet new people',hint:'Find my place at school.',outcome:'I started high school and met new people.',effect:{Happiness:3},schoolEffect:{popularity:4}}]};
