...
export type DanceMode='classmate'|'friends'|'alone';
...
export function schoolDance(life:Life,mode:DanceMode,personId?:string):DanceOutcome {
 ...
 const companions:Person[]=mode==='classmate'?[classmate!]:mode==='friends'?friends:[];
 ...
 const accepted=mode==='alone' || mode==='classmate' && compatible && random()<Math.min(.95,.15+strength/120) || mode==='friends' && friends.length>0 && random()<strength/100;
 ...
 const happiness=accepted?mode==='alone'?Math.round((yourEnjoyment-45)/10):1+Math.floor(yourEnjoyment/15):-3;
 const gain=accepted?1+Math.floor(theirEnjoyment/20):-2;
 ...
 const text=!accepted?mode==='classmate'?`${classmate!.name} declined your invitation to the school dance.`:friends.length?'Your friends could not agree to go to the school dance with you.':'You do not have any befriended contacts to invite yet.':mode==='classmate'?`You went to the school dance with ${classmate!.name}. You danced, talked, and made a memory together.`:mode==='friends'?'You went to the school dance with your friends. You spent the evening together.':'You went to the school dance alone and enjoyed an evening of music and dancing.';
 ...
 return {life:next,accepted,text,bars:accepted?[{label:'Your Enjoyment',value:yourEnjoyment},...(mode==='alone'?[]:[{label:mode==='friends'?"Your Friends’ Enjoyment":classmate!.gender==='Female'?'Her Enjoyment':'His Enjoyment',value:theirEnjoyment}])]:[],retry:!accepted};
}
