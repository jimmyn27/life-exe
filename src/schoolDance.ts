...
export function schoolDance(life:Life,mode:DanceMode,personId?:string):DanceOutcome {
 ...
 const companions:Person[]=mode==='partner'?[partner!]:mode==='classmate'?[classmate!]:mode==='friends'?friends:[];
 ...
 const accepted=mode==='partner' || mode==='alone' || mode==='classmate' && compatible && random()<Math.min(.95,.15+strength/120) || mode==='friends' && friends.length>0 && random()<strength/100;
 ...
 const text=!accepted?mode==='classmate'?`${classmate!.name} declined your invitation to the school dance.`:friends.length?'Your friends could not agree to go to the school dance with you.':'You do not have any befriended contacts to invite yet.':mode==='partner'?`You went to the school dance with your ${partner!.gender==='Female'?'girlfriend':'boyfriend'}, ${partner!.name}. You danced, talked, and made a memory together.`:mode==='classmate'?`You went to the school dance with ${classmate!.name}. You danced, talked, and made a memory together.`:mode==='friends'?'You went to the school dance with your friends. You spent the evening together.':'You went to the school dance alone and enjoyed an evening of music and dancing.';
