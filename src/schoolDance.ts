...
export function schoolDance(life:Life,mode:DanceMode,personId?:string):DanceOutcome {
 ...
 const charismaDirection=mode==='alone'?0:accepted?1:-1;
 const charismaChange=charismaDirection!==0&&seededRandom(life.id+':dance:'+life.age+':'+mode+':'+(personId??'')+':charisma')()<.5?charismaDirection:0;
 const next={...life,stats:{...life.stats,Happiness:clamp(life.stats.Happiness+happiness),Charisma:clamp(life.stats.Charisma+charismaChange)},relationships,occupation:{...occupation,school:{...school,...(mode==='classmate'?{danceAskedIds:[...(school.danceAskedIds??[]),personId!]}:{}),yearActions:accepted?[...(school.yearActions??[]),'School dance' as const]:school.yearActions}},log:[...life.log,{age:life.age,tag:'EDUCATION',text:text.replace(/^You /,'I ').replace(/your /g,'my ').replace(/You danced/,'I danced').replace(/You spent/,'I spent')}]};
