...
export function advanceSchoolRoster(id:string,age:number,previous:readonly SchoolPerson[]=[],looks=50):SchoolPerson[]{
...
export function initialSchoolRoster(id:string,age:number,looks=50):SchoolPerson[]{
 let roster:SchoolPerson[]=[];
 for(let year=6;year<=Math.min(age,17);year++)roster=advanceSchoolRoster(id,year,roster,looks);
 return age>=18?advanceSchoolRoster(id,age,[],looks):roster;
}
