...
export function advanceSchoolRoster(id:string,age:number,previous:readonly SchoolPerson[]=[]):SchoolPerson[]{
 ...
 const make=(index:number,relation:SchoolPerson['relation'],subject?:string):SchoolPerson=>{
  ...
  used.add(name);
  return {id:`school-${stage}-${age}-${relation}-${index}`,name,gender,ageOffset:relation==='Classmate'?0:relation==='Principal'?integer(30,45):integer(20,38),relation,group:relation==='Classmate'?'Classmates':'Staff',strength:integer(40,75),...(subject?{subject}:{})};
 };
...
export function initialSchoolRoster(id:string,age:number):SchoolPerson[]{
 ...
 for(let year=6;year<=Math.min(age,17);year++)roster=advanceSchoolRoster(id,year,roster);
 return age>=18?advanceSchoolRoster(id,age):roster;
}
