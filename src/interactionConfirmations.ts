...
export function interactionConfirmation(life:Life,person:Person,action:RelationshipAction):LifeEvent|null{
 ...
 const title=action==='Befriend'?`Become friends with ${target}?`:action==='Ask out'?`A date with ${target}?`:action==='Insult'?`Insult ${target}?`:action==='Flirt'?`Flirt with ${target}?`:action==='Break up'?`End your relationship with ${target}?`:action==='Suck up'?`Try to impress ${target}?`:action==='Have fun'?`Spend an afternoon with ${target}?`:`An intimate invitation for ${target}?`;
 return {category:'Relationship',title,text,icon:interactionEmojis[action],...(['Befriend','Ask out','Hook up','Have fun'].includes(action)?{profileId:person.id}:{}),choices:[{label:action==='Have fun'?`Try to have fun with ${pronoun}`:interactionLabels[action],hint:'See how they respond.',outcome:'',relationship:{id:person.id,action}},{label:'Nevermind',hint:'Return to their profile.',outcome:''}]};
}
