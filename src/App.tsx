...
export default function App() {
  ...
  function activity(id:'book'|'walk'){
    ...
    setLife({...life,stats,log:[...life.log,{age:life.age,tag:'ACTIVITY',text:outcome}]});setDirty(true);
  ...
  function finishMembershipAction(id:string,action:MembershipAction) {const next=manageSchoolActivity(life,id,action);setNoticeTitle(action==='Quit'?'Leaving an extracurricular':['Extra effort','Working on my performance','Putting in the hours'][Math.floor(Math.random()*3)]);setNoticeIcon(schoolActivities.find(item=>item.id===id)?.emoji??'🏅');if(next!==life){setLife(next);setDirty(true);setNotice(next.log.at(-1)?.text.replace(/^I /,'You ')??'');}else setNotice('You put in more effort, but you already received the performance benefit this year.');}
...
function finishJobOffer(id:string){const next=takePartTimeJob(life,id);if(next!==life){const acceptedJob=occupationJobs(getOccupation(next)).find(job=>job.id===id);setLife(next);setDirty(true);setNoticeTitle(['Application submitted',`Applying for ${acceptedJob?.position??'a job'}`,'A new job opportunity'][Math.floor(Math.random()*3)]);setNoticeIcon(jobEmoji(id));setNotice(next.log.at(-1)?.text??'');}}
  ...
  return <div className={`classic-desktop managed-desktop luna-desktop ${mode !== 'desktop' ? 'session-screen' : ''}`}>
    ...
    <dialog ref={modalRef} onCancel={closeModal} className={`classic-window event-dialog ${modal === 'power' ? 'power-dialog' : ''}`} aria-label={dialogTitle}>
      ...
      {notice ? <>
        <div className="modal-content information-content"><span className="popup-emoji" aria-hidden="true">{noticeIcon}</span><p>{notice}</p></div>{noticeMeter&&<div className="result-reaction"><strong>{noticeMeter.label}</strong><div className="result-reaction-track" role="progressbar" aria-label={noticeMeter.label} aria-valuenow={noticeMeter.value} aria-valuemin={0} aria-valuemax={100}><div style={{width:`${noticeMeter.value}%`,background:barColor(noticeMeter.value)}}/></div></div>}<div className="dialog-actions"><button className="classic-button" onClick={closeModal}>OK</button></div>
