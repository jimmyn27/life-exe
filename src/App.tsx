...
import {takePartTimeJob,partTimeOffers,projectedJobHours,workAction,jobEmoji,type WorkAction, type HoursDirection} from './partTimeWork';
...
import { generateFamily, seededRandom } from './family';
import { schoolAction,getOccupation,occupationJobs,nextSchoolName,schoolName, type SchoolAction } from './occupation';
...
function eventEmoji(event:LifeEvent):string {
  if(event.icon)return event.icon;
  const category=event.category.toLowerCase();
  if(category.includes('compliment'))return '😘';
  if(category.includes('insult'))return '🤬';
  if(category.includes('gift'))return '🎁';
  if(category.includes('relationship')||category.includes('friend'))return '🤝';
  if(category.includes('school')||category.includes('education'))return '📚';
  if(category.includes('job')||category.includes('employment'))return '💼';
  if(category.includes('sport')||category.includes('extracurricular'))return '🏅';
  if(category.includes('family'))return '🏠';
  if(category.includes('invitation'))return '💌';
  return '✨';
}
...
export default function App() {
  ...
  const [noticeIcon,setNoticeIcon]=useState(boot.error?'⚠️':'ℹ️');
  const [noticeMeter,setNoticeMeter]=useState<{label:string;value:number}|null>(null);
  ...
  const [draftGender,setDraftGender]=useState<'Female'|'Male'>('Male');
  ...
  const [webTarget,setWebTarget]=useState<{page:'Education'|'Work';version:number}|undefined>();
  ...
  function closeModal() { setDanceOpen(false);setDanceOutcome(null); setResult(null); setDeleteId(null); setEvent(null); setModal(null); setNotice('');setNoticeTitle('Life.exe');setNoticeIcon('ℹ️');setNoticeMeter(null); }
  function saveCurrent(announce = false): boolean {
    ...
    try {
      ...
      if (announce) { setModal(null); setNoticeTitle('Life saved');setNoticeIcon('💾'); setNotice(`${life.name}'s life has been saved on this device. Age ${life.age} · Year ${life.age}.`); }
      return true;
    } catch { setModal(null); setNoticeTitle('Save failed');setNoticeIcon('⚠️'); setNotice('Your life could not be saved. Browser storage may be unavailable or full. Your current progress is still open; please do not turn off or reload until saving works.'); return false; }
  }
  ...
  function deleteCharacter() {
    ...
    try {
      ...
      closeModal();
    } catch { setModal(null); setDeleteId(null); setNoticeTitle('Delete failed');setNoticeIcon('⚠️'); setNotice('This life could not be deleted because browser storage is unavailable. Your saved lives have not been changed.'); }
  }
  function switchCharacter(saved: Life) {
    ...
    } catch { setNoticeTitle('Character could not be opened');setNoticeIcon('⚠️'); setNotice('The character could not be opened because browser storage is unavailable. Your saved characters have not been changed.'); }
  }
  function showNewLife() { setDraftGender('Male'); setDraftSexuality('Straight'); setDraftCountry('United States'); setDraftFirstName(''); setDraftLastName(''); setDraftCity(life.locationId ?? resolveCity(life.city)?.id ?? DEFAULT_CITY_ID); setModal('new'); setMenu(false); }
  function startLife(e: FormEvent) {
    ...
    const family = generateFamily(id, lastName, draftGender);
  ...
  function choose(choice: Choice, index: number) {
    ...
    if(choice.workAction){setEvent(null);finishWorkAction(choice.workAction.jobId,choice.workAction.action,choice.workAction.direction);return;}
    ...
    if (choice.relationship) {
      ...
      finishInteraction(id,action,giftId,invited);
  ...
  function activity(id:'book'|'walk'){
    if(id==='book'&&life.age<6||id==='walk'&&life.age<10)return;
    const used=life.activityActions?.age===life.age?life.activityActions.ids:[];
    if(used.includes(id)){const repeat=id==='book'?['You could not find another book you wanted to read.','Nothing else on the shelf caught your interest.','You decided to save your next book for another year.','You had read enough for now and could not focus on another story.']:['You did not feel like taking another walk.','Your legs needed a rest before another long walk.','You decided one walk was enough for today.','The thought of another walk did not appeal to you.'];const random=seededRandom(life.id+':'+life.age+':'+id+':repeat');setNoticeTitle(id==='book'?'No book caught your eye':'Not another walk');setNoticeIcon(id==='book'?'📚':'👟');setNotice(repeat[Math.floor(random()*repeat.length)]);setNoticeMeter(null);return;}
    const stat=id==='book'?'Intelligence':'Health',random=seededRandom(life.id+':'+life.age+':'+id+':'+life.log.length),current=life.stats[stat];
    const enjoyment=Math.max(0,Math.min(100,Math.round(current*.65+random()*35)));
    const maximumGain=Math.max(1,Math.ceil((100-current)/20)),statGain=1+Math.floor(random()*Math.min(5,maximumGain)),happinessGain=1+Math.round(enjoyment*.09);
    const stats={...life.stats,[stat]:Math.min(100,current+statGain),Happiness:Math.min(100,life.stats.Happiness+happinessGain)};
    const bookLines=['I found a novel I could not put down.','I spent the afternoon lost in a good book.','I discovered a story that kept me turning pages.','I curled up somewhere quiet and read for hours.','I picked a book from the shelf and learned something new.'];
    const walkLines=['I went for a peaceful walk around the neighborhood.','I took a long walk and enjoyed the fresh air.','I explored a nearby trail on foot.','I stretched my legs with a walk through the park.','I wandered outside until my mind felt clearer.'];
    const lines=id==='book'?bookLines:walkLines,outcome=lines[Math.floor(random()*lines.length)];
    setLife({...life,activityActions:{age:life.age,ids:[...used,id]},stats,log:[...life.log,{age:life.age,tag:'ACTIVITY',text:outcome}]});setDirty(true);
    setNoticeTitle(id==='book'?['A good book','Turning the page','Lost in a story','Reading for pleasure'][Math.floor(random()*4)]:['Going for a walk','Fresh air','A peaceful stroll','Stretching my legs'][Math.floor(random()*4)]);
    setNoticeIcon(id==='book'?'📚':'👟');setNotice(outcome.replace(/^I /,'You '));setNoticeMeter({label:'Enjoyment',value:enjoyment});
  }
  function performSchoolAction(action:SchoolAction) {
    const currentSchool=getOccupation(life).school;if(action==='Change schools'&&currentSchool&&currentSchool.level!=='University'&&currentSchool.transferStages?.includes(currentSchool.level)){setNoticeTitle('Change schools');setNoticeIcon('📖');setNotice("Your parents won’t let you change schools again during this stage of your education.");return;}
    if(action==='School dance'){setDanceOutcome(null);setDanceOpen(true);return;}
    if(action==='Drop out' && life.age<16){setNoticeTitle('Too young to drop out');setNoticeIcon('👋');setNotice('You must be at least 16 years old to drop out of school.');return;}
    if(action==='Drop out' || action==='Change schools' || action==='Skip school'){
      setEventSource({kind:'activity'});setEvent({category:'School',title:action,icon:action==='Change schools'?'📖':action==='Drop out'?'👋':action==='Skip school'?'🤫':'🏫',text:action==='Drop out'?'Are you sure you want to drop out of school?':action==='Skip school'?'Are you sure you want to skip school?':`Ask your parents to send you to a different school?\nCurrent school: ${schoolName(life,getOccupation(life).school!)}\nNew school: ${nextSchoolName(life,getOccupation(life).school!)}`,choices:[{label:action==='Change schools'?'Ask my parents':action,hint:'See what happens.',outcome:'',schoolAction:action},{label:'Nevermind',hint:'Return to school.',outcome:''}]});return;
    }
    finishSchoolAction(action);
  }
  function finishSchoolAction(action:SchoolAction) {
    const already=Boolean(getOccupation(life).school?.yearActions?.includes(action)||action==='Study harder'&&getOccupation(life).school?.yearActions?.includes('Study hard'));
    const next=schoolAction(life,action);if(next!==life){setLife(next);setDirty(true);}
    if(action==='Study harder'){const titles=['Studying harder','Hitting the books','Extra study time','A productive study session','Reviewing my notes','One more chapter'];const messages=['You studied until the library closed.','You reviewed your notes until late in the evening.','You spent the afternoon working through difficult questions.','You studied until your pencil needed sharpening again.','You made flashcards and reviewed them after dinner.','You worked through another stack of practice questions.'];const pick=Math.floor(Math.random()*messages.length);setNoticeTitle(titles[pick%titles.length]);setNoticeIcon('📚');setNotice(already?(next.log.at(-1)?.text.replace(/^I /,'You ')??'You should wait before studying harder again.'):messages[pick]);}
    else {setNoticeTitle(action==='Nurse'?'A visit to the school nurse':action);setNoticeIcon(action==='Nurse'?'🩺':action==='Skip school'?'🤫':action==='Change schools'?'📖':'👋');setNotice(next!==life?next.log.at(-1)?.text.replace(/^I /,'You ').replace(/My /g,'Your ')??'':'You already did this school action this year.');}
  }
  function membershipAction(id:string,action:MembershipAction,hours?:number) {
    ...
    if(action==='Quit'){const activity=schoolActivities.find(item=>item.id===id);if(!activity)return;setEventSource({kind:'activity'});setEvent({category:'Extracurricular',icon:activity.emoji,title:`Leave ${activity.emoji} ${schoolActivityName(activity,life.family?.gender)}?`,text:`Are you sure you want to quit this ${activity.group==='Sports'?'team':'club'}?`,choices:[{label:activity.group==='Sports'?'Quit the team':'Quit the club',hint:'Leave this extracurricular.',outcome:'',membershipAction:{id,action}},{label:'Nevermind',hint:'Remain a member.',outcome:''}]});return;}
    finishMembershipAction(id,action);
  }
  function finishMembershipAction(id:string,action:MembershipAction) {const next=manageSchoolActivity(life,id,action);setNoticeTitle(action==='Quit'?'Leaving an extracurricular':['Extra effort','Working on my performance','Putting in the hours'][Math.floor(Math.random()*3)]);setNoticeIcon(schoolActivities.find(item=>item.id===id)?.emoji??'🏅');if(next!==life){setLife(next);setDirty(true);setNotice(next.log.at(-1)?.text.replace(/^I /,'You ')??'');}else setNotice(action==='Train'?'You have already pushed yourself as far as you can this year.':'That action is not available right now.');}
  ...
  function relationshipAction(id:string,action:RelationshipAction){
    ...
    if(action==='Gift'&&life.relationships?.[id]?.usedAge===life.age&&life.relationships[id].usedActions?.includes('Gift')){finishInteraction(id,action);return;}
  ...
  function applyActivity(id:string){const activity=schoolActivities.find(item=>item.id===id);if(!activity)return;if(scheduleHours(life)+5>scheduleLimit){setNoticeTitle('Schedule full');setNoticeIcon('⏳');setNotice(busyMessage);return;}const name=schoolActivityName(activity,life.family?.gender);setEventSource({kind:'activity'});setEvent({category:activity.group==='Sports'?'Sports tryout':'Club application',icon:activity.emoji,title:`${activity.emoji} ${name}`,text:activity.group==='Sports'?'Try out today!':'Apply today!',meters:activity.group==='Sports'?[{name:'Health',value:life.stats.Health}]:[{name:'Intelligence',value:life.stats.Intelligence},{name:'Charisma',value:life.stats.Charisma}],choices:[{label:activity.group==='Sports'?'Try out':'Apply',hint:activity.group==='Sports'?'Attend the team tryout.':'Submit your application.',outcome:'',schoolActivityDecision:{id}},{label:'Nevermind',hint:'Return to the list.',outcome:''}]});}
function finishApplyActivity(id:string){const next=applySchoolActivity(life,id);if(next!==life){setLife(next);setDirty(true);const activity=schoolActivities.find(item=>item.id===id);setNoticeTitle(activity?[`${activity.group==='Sports'?'Tryout':'Application'} result`,`News from ${schoolActivityName(activity,life.family?.gender)}`,activity.group==='Sports'?'The team has decided':'The club has decided'][Math.floor(Math.random()*3)]:'Application result');setNoticeIcon(activity?.emoji??'📋');setNotice(next.log.at(-1)?.text??'');}}
  function performWorkAction(jobId:string,action:WorkAction){
    const job=occupationJobs(getOccupation(life)).find(item=>item.id===jobId);if(!job)return;setEventSource({kind:'activity'});
    if(action==='Hours'){setEvent({category:'Employment',icon:'⌛',title:'Change hours at '+job.employer,text:'How would you like to change your weekly hours?',choices:[{label:'Ask for more hours',hint:'',outcome:'',workAction:{jobId,action,direction:'more'}},{label:'Ask for fewer hours',hint:'',outcome:'',workAction:{jobId,action,direction:'fewer'}},{label:'Nevermind',hint:'',outcome:''}]});return;}
    if(action==='Resign'){setEvent({category:'Employment',icon:'👋',title:'Resign from '+job.position+'?',text:'Are you sure you want to leave '+job.employer+'?',choices:[{label:'Tender my resignation',hint:'',outcome:'',workAction:{jobId,action}},{label:'Nevermind',hint:'',outcome:''}]});return;}
    finishWorkAction(jobId,action);
  }
  function finishWorkAction(jobId:string,action:WorkAction,direction:HoursDirection='more'){const result=workAction(life,jobId,action,direction);if(result.life!==life){setLife(result.life);setDirty(true);}setNoticeTitle(action==='Work harder'?['Working harder','Putting in extra effort','A productive shift','Going the extra mile'][Math.floor(Math.random()*4)]:action==='Raise'?['Raise request','Asking for a raise','Talking about pay','Making my case'][Math.floor(Math.random()*4)]:['Hours request','Changing my schedule','Talking about hours','A new work schedule'][Math.floor(Math.random()*4)]);setNoticeIcon(action==='Work harder'?'💪':action==='Raise'?'💸':'⌛');setNotice(result.text);}
  ...
  function showJobOffer(id:string){const offer=partTimeOffers(life).find(j=>j.id===id);if(!offer)return;if(projectedJobHours(life,offer.weeklyHours)>scheduleLimit){setNoticeTitle('Schedule full');setNoticeIcon('⏳');setNotice(busyMessage);return;}setEventSource({kind:'activity'});setEvent({category:'Job application',icon:jobEmoji(offer.id),title:`${jobEmoji(offer.id)} ${offer.title}`,text:`Employer: ${offer.title==='Babysitter'||offer.title==='Pet sitter'?'Neighborhood families':'Local '+offer.title.replace(/ worker| assistant| aide| attendant/i,'')}
...
function finishJobOffer(id:string){const next=takePartTimeJob(life,id);if(next!==life){const acceptedJob=occupationJobs(getOccupation(next)).find(job=>job.id===id);setLife(next);setDirty(true);setNoticeTitle(['Application submitted',`Applying for ${acceptedJob?.position??'a job'}`,'A new job opportunity'][Math.floor(Math.random()*3)]);setNoticeIcon(jobEmoji(id));setNotice(next.log.at(-1)?.text??'');}}
  ...
  function about() { setMenu(false); setNoticeTitle('About Life.exe');setNoticeIcon('ℹ️'); setNotice('Life.exe — Luna edition. Enternet Explorer is for activities, jobs, and education. My Life is your character overview and statistics monitor. File Explorer holds your assets and finances. Messenger is for relationships. Yearly life events appear in pop-ups. Command Prompt records your story. Saves are stored locally in this browser. The date advances once per life year; the world rules remain fixed.'); }
  ...
  return <div className={`classic-desktop managed-desktop luna-desktop ${mode !== 'desktop' ? 'session-screen' : ''}`}>
    {mode === 'desktop' ? <>
      <div className="desktop-workspace" ref={workspaceRef}>
        ...
        {windows && windowIds.filter(id => windows[id].status !== 'closed').map(id => <FloatingWindow key={`${life.id}:${id}`} id={id} title={titles[id]} icon={icons[id]} state={windows[id]} bounds={bounds} active={activeId === id} className={`program-window program-${id.toLowerCase()}`} onFocus={() => focusWindow(id)} onChange={rect => setRect(id, rect)} onMinimize={() => minimize(id)} onMaximize={() => toggleMaximize(id)} onClose={() => closeWindow(id)}>
          {id === 'Command' ? <CommandPrompt life={life} feedRef={feedRef}/> : id === 'Explorer' ? <FileExplorer life={life} page={explorerPage} onPage={setExplorerPage} onSave={() => saveCurrent(true)} onNotice={setNotice}/> : id === 'Life' ? <MyLife life={life} dirty={dirty} onContact={openProfile} onFinances={() => { setExplorerPage('Finances'); openWindow('Explorer'); }}/> : id === 'Web' ? <WebSurfer life={life} requestedPage={webTarget} onWorkAction={performWorkAction} onOpenLife={openMyLife} onTakeJob={showJobOffer} onSchedule={()=>setModal('schedule')} onFinances={()=>{setExplorerPage('Finances');openWindow('Explorer');}} onMembership={membershipAction} onAction={relationshipAction} onActivity={activity} onNotice={setNotice} onSave={() => saveCurrent(true)} onSchoolAction={performSchoolAction} onApplyActivity={applyActivity}/> : <Messenger life={life} profileId={messengerProfile} profileVersion={profileVersion} onAction={relationshipAction} onOpenLife={openMyLife}/>}
        </FloatingWindow>)}
    ...
    <dialog ref={modalRef} onCancel={closeModal} className={`classic-window event-dialog ${modal === 'power' ? 'power-dialog' : ''}`} aria-label={dialogTitle}>
      ...
      {notice ? <>
        ...
        <div className="modal-content"><label htmlFor="character-first-name">First name:</label><input id="character-first-name" value={draftFirstName} onChange={e=>setDraftFirstName(e.target.value)} maxLength={48} autoComplete="given-name" required/><label htmlFor="character-last-name">Last name:</label><input id="character-last-name" value={draftLastName} onChange={e=>setDraftLastName(e.target.value)} maxLength={48} autoComplete="family-name" required/><label htmlFor="character-gender">Gender:</label><select id="character-gender" value={draftGender} onChange={e=>setDraftGender(e.target.value as 'Female'|'Male')}><option>Female</option><option>Male</option></select><label htmlFor="character-sexuality">Sexuality:</label><select id="character-sexuality" value={draftSexuality} onChange={e=>setDraftSexuality(e.target.value as Sexuality)}>{(['Straight','Bisexual','Gay'] as const).map(value=><option key={value}>{value}</option>)}</select><label htmlFor="character-country">Country:</label><select id="character-country" value={draftCountry} onChange={e=>setDraftCountry(e.target.value)}><option>United States</option></select><label htmlFor="character-city">City:</label><select id="character-city" value={draftCity} onChange={e=>setDraftCity(e.target.value)}>{cityOptions.map(city=><option key={city.id} value={city.id}>{city.name}</option>)}</select><p className="modal-note">{mode==='desktop'?"Your current character is saved before a new life starts.":"Create your character to begin."}</p></div><div className="dialog-actions"><button className="classic-button" type="submit">Start Life</button><button className="classic-button" type="button" onClick={closeModal}>Nevermind</button></div>
      </form> : event && <div className="modal-content"><div className="popup-message"><span className="popup-emoji" aria-hidden="true">{eventEmoji(event)}</span><p className="event-description" style={{whiteSpace:'pre-line'}}>{event.text}</p></div>{event.meters?.map(meter=><div className="result-reaction" key={meter.name}><strong>{meter.name}</strong><div className="result-reaction-track" role="progressbar" aria-label={meter.name} aria-valuenow={meter.value} aria-valuemin={0} aria-valuemax={100}><div style={{width:`${meter.value}%`,background:barColor(meter.value)}}/></div></div>)}{event.profileId && <ConfirmationProfile life={life} id={event.profileId}/>}{event.acknowledge?<div className="dialog-actions"><button className="classic-button" onClick={()=>choose(event.choices[0],0)}>OK</button></div>:<fieldset className="event-choices"><legend>What will you do?</legend>{event.choices.map((choice,index)=><button className="classic-button choice-button" key={choice.label} disabled={choice.disabled} onClick={()=>choose(choice,index)}><strong>{choice.label}</strong></button>)}</fieldset>}</div>}
    </dialog>
  </div>;
}
