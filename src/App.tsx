...
export default function App() {
  ...
  function closeModal() { setDanceOpen(false);setDanceOutcome(null); setResult(null); setDeleteId(null); setEvent(null); setModal(null); setNotice('');setNoticeTitle('Life.exe');setNoticeIcon('ℹ️'); }
  ...
  function activity(title:string,_text:string,outcome:string,effect:Partial<Stats>){const stats={...life.stats};for(const key of Object.keys(effect) as (keyof Stats)[])stats[key]=Math.max(0,Math.min(100,stats[key]+(effect[key]??0)));setLife({...life,stats,log:[...life.log,{age:life.age,tag:'ACTIVITY',text:outcome}]});setDirty(true);const activityTitles:Record<string,string[]>={'Play with toys':['Play time','Fun with toys','A little playtime'],'Story time':['Story time','Once upon a time','A bedtime story'],'Take a walk':['Taking a walk','Out for a stroll','Fresh air'],'Read a book':['Reading time','A good book','Turning the page'],'Try a creative hobby':['Getting creative','A creative afternoon','Trying something new']};const pool=activityTitles[title]??[title];setNoticeTitle(pool[Math.floor(Math.random()*pool.length)]);setNoticeIcon(title==='Play with toys'?'🧸':title==='Story time'||title==='Read a book'?'📚':title==='Take a walk'?'🚶':'🎨');setNotice(outcome.replace(/^I /,'You '));}
  ...
  function finishSchoolAction(action:SchoolAction) {
    ...
    if(action==='Study harder'){const titles=['Studying harder','Hitting the books','Extra study time','A productive study session'];const messages=['You studied until the library closed.','You reviewed your notes until late in the evening.','You spent the afternoon working through difficult questions.','You studied until your pencil needed sharpening again.','You made flashcards and reviewed them after dinner.'];const pick=Math.floor(Math.random()*messages.length);setNoticeTitle(titles[pick%titles.length]);setNoticeIcon('📚');setNotice(messages[pick]);}
...
function finishApplyActivity(id:string){const next=applySchoolActivity(life,id);if(next!==life){setLife(next);setDirty(true);const activity=schoolActivities.find(item=>item.id===id);setNoticeTitle(activity?[`${activity.group==='Sports'?'Tryout':'Application'} result`,`News from ${schoolActivityName(activity,life.family?.gender)}`,activity.group==='Sports'?'The team has decided':'The club has decided'][Math.floor(Math.random()*3)]:'Application result');setNoticeIcon(activity?.emoji??'📋');setNotice(next.log.at(-1)?.text??'');}}
  function performWorkAction(action:WorkAction){if(action==='Resign'){const job=getOccupation(life).job;if(!job)return;setEventSource({kind:'activity'});setEvent({category:'Employment',icon:'👋',title:`Resign from ${job.position}?`,text:`Are you sure you want to leave ${job.employer}?`,choices:[{label:'Tender my resignation',hint:'Leave this job.',outcome:'',workAction:action},{label:'Nevermind',hint:'Keep working here.',outcome:''}]});return;}finishWorkAction(action);}
  function finishWorkAction(action:WorkAction){const result=workAction(life,action);if(result.life!==life){setLife(result.life);setDirty(true);}setNoticeTitle(action==='Work harder'?['Working harder','Putting in extra effort','A productive shift'][Math.floor(Math.random()*3)]:action==='Raise'?['Raise request','Asking for a raise','Talking about pay'][Math.floor(Math.random()*3)]:['Hours request','Changing my schedule','Talking about hours'][Math.floor(Math.random()*3)]);setNoticeIcon(action==='Work harder'?'💪':action==='Raise'?'💸':'⏳');setNotice(result.text);}
...
function finishJobOffer(id:string){const next=takePartTimeJob(life,id);if(next!==life){setLife(next);setDirty(true);setNoticeTitle(['Application submitted',`Applying for ${next.occupation?.job?.position??'a job'}`,'A new job opportunity'][Math.floor(Math.random()*3)]);setNoticeIcon(jobEmoji(id));setNotice(next.log.at(-1)?.text??'');}}
  ...
  return <div className={`classic-desktop managed-desktop luna-desktop ${mode !== 'desktop' ? 'session-screen' : ''}`}>
    ...
    <dialog ref={modalRef} onCancel={closeModal} className={`classic-window event-dialog ${modal === 'power' ? 'power-dialog' : ''}`} aria-label={dialogTitle}>
      ...
      {notice ? <>
        <div className="modal-content information-content"><span className="popup-emoji" aria-hidden="true">{noticeIcon}</span><p>{notice}</p></div><div className="dialog-actions"><button className="classic-button" onClick={closeModal}>OK</button></div>
