...
export default function App() {
  ...
  function relationshipAction(id: string, action: RelationshipAction) {
    ...
    if(action==='Gift'){setEvent({category:'Gift',title:`Gift · ${person.name}`,text:'Choose a gift. Their response depends on its value and how appropriate it is.',choices:[...giftOptions().map(gift=>({label:`${gift.name} ($${gift.price})`,hint:life.balance<gift.price?'You cannot afford this gift.':'Give this gift.',disabled:life.balance<gift.price,outcome:'',relationship:{id,action,giftId:gift.id}})),{label:'Cancel',hint:'Return to their profile.',outcome:''}]});return;}
    setEvent({category:'Relationship',title:`${action} · ${person.name}`,text:action === 'Befriend' ? `Become friends with ${person.name}?` : action === 'Ask for money' ? `Ask ${person.name} for money? You can ask each parent once this year.` : action === 'Unfriend' ? `End your friendship with ${person.name}?` : action === 'Ask out' ? `Ask ${person.name} out on a date?` : action === 'Compliment' ? `Give ${person.name} a sincere compliment?` : action === 'Insult' ? `Insult ${person.name}? This may hurt your relationship.` : action === 'Conversation' ? `Have a conversation with ${person.name}?` : `Would you like to ${action.toLowerCase()} with ${person.name}?`,choices:[{label:action,hint:'See how they respond.',outcome:'',relationship:{id,action}},{label:'Cancel',hint:'Return to their profile.',outcome:''}]});
  }
    ...
    {mode === 'desktop' ? <>
      <div className="desktop-workspace" ref={workspaceRef}>
        ...
        {windows && windowIds.filter(id => windows[id].status !== 'closed').map(id => <FloatingWindow key={`${life.id}:${id}`} id={id} title={titles[id]} icon={icons[id]} state={windows[id]} bounds={bounds} active={activeId === id} className={`program-window program-${id.toLowerCase()}`} onFocus={() => focusWindow(id)} onChange={rect => setRect(id, rect)} onMinimize={() => minimize(id)} onMaximize={() => toggleMaximize(id)} onClose={() => closeWindow(id)}>
          {id === 'Command' ? <CommandPrompt life={life} feedRef={feedRef}/> : id === 'Explorer' ? <FileExplorer life={life} page={explorerPage} onPage={setExplorerPage} onSave={() => saveCurrent(true)} onNotice={setNotice}/> : id === 'Life' ? <MyLife life={life} dirty={dirty} onFinances={() => { setExplorerPage('Finances'); openWindow('Explorer'); }}/> : id === 'Web' ? <WebSurfer life={life} onActivity={activity} onNotice={setNotice} onSave={() => saveCurrent(true)} onSchoolAction={(action:SchoolAction)=>{setLife(previous=>schoolAction(previous,action));setDirty(true);}} onApplyActivity={id=>{const next=applySchoolActivity(life,id);if(next===life)return;setLife(next);setDirty(true);setNotice(next.log.at(-1)?.text??'');}}/> : <Messenger life={life} onAction={relationshipAction} onOpenLife={openMyLife}/>}
        </FloatingWindow>)}
