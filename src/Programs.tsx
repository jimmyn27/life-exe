...
export function CommandPrompt({ life, feedRef }: { life: Life; feedRef: RefObject<HTMLDivElement | null> }) {
  ...
  return <div className="command-screen" ref={feedRef} role="log" aria-label="Life history">{ages.map(age => <section className="command-entry" key={age}><h2 className="command-age">Age {age}</h2>{life.log.filter(entry => entry.age === age).map((entry, i) => <p key={i}>{entry.text}</p>)}</section>)}</div>;
}
