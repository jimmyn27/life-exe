...
export function advanceCommitments(before:Life,after:Life):Life {
 ...
 for(const id of school.memberships??[]){const activity=schoolActivities.find(a=>a.id===id);if(!activity)continue;const old=membershipInfo(before.occupation?.school??getOccupation(before).school??school,id,before.age);const performance=clamp(old.performance+(old.hours-5)*2-penalty),years=after.age-old.joinAge;
