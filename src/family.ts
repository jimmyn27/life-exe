...
export function generateFamily(id: string, surname: string, gender?: 'Female' | 'Male'): Family {
 const random=seededRandom(id+':family');
 const integer=(a:number,b:number)=>a+Math.floor(random()*(b-a+1));
 const stats=():Stats=>({Health:integer(65,100),Happiness:integer(55,95),Intelligence:integer(30,95),Appearance:integer(30,95)});
 ...
 const inherit=(key:'Health'|'Intelligence'|'Appearance')=>clamp(Math.round(parents.reduce((sum,p)=>sum+p.stats[key],0)/parents.length)+integer(-8,8));
 ...
 return {parents,siblings,money,gender:gender??(random()<.5?'Female':'Male'),planned:random()<.65,birthStats:{Health:inherit('Health'),Happiness:integer(70,95),Intelligence:inherit('Intelligence'),Appearance:inherit('Appearance')},origin:structuredClone({parents,money,siblings})};
}
...
export function advanceFamily(family: Family | undefined,id:string,age:number,surname:string): {family:Family|undefined; newborn?:Sibling; promotions:Parent[]} {
 ...
 if(mother && random()<siblingBirthChance(mother.ageAtBirth+age,siblings.length+1)) {
  ...
  newborn={id:`sibling-born-${age}`,name:siblingName(gender,surname,random,new Set([...parents,...siblings].map(person=>person.name))),gender,birthAge:age,stats:{Health:family.birthStats.Health,Happiness:80,Intelligence:family.birthStats.Intelligence,Appearance:family.birthStats.Appearance}};
