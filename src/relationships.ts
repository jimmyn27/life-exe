...
export const relationshipActions = ['Befriend', 'Ask for money', 'Ask out', 'Compliment', 'Conversation', 'Gift', 'Hook up', 'Insult', 'Spend time', 'Unfriend'] as const;
...
export function availableActions(person: Person, life?: Life): RelationshipAction[] {
 ...
 return relationshipActions.filter(action=>action!=='Ask for money' && (action!=='Befriend' || !person.friendship) && (action!=='Unfriend' || person.friendship));
}
...
export function interact(life: Life, id: string, action: RelationshipAction): Life {
  ...
  const deltas: Record<RelationshipAction,number> = {Befriend:5,'Ask for money':0,'Ask out':accepted ? 5 : -2,Compliment:4,Conversation:3,Gift:6,'Hook up':accepted ? 2 : -2,Insult:-12,'Spend time':5,Unfriend:0};
