...
export const relationshipActions = ['Befriend', 'Ask for money', 'Ask out', 'Compliment', 'Conversation', 'Flirt', 'Gift', 'Insult', 'Hook up', 'Make love', 'Spend time', 'Unfriend', 'Act up', 'Disrespect', 'Suck up'] as const;
...
export function availableActions(person: Person, life?: Life): RelationshipAction[] {
 ...
 return relationshipActions.filter(action=>!['Act up','Disrespect','Suck up'].includes(action) && action!=='Ask for money' && (action!=='Make love' || person.status==='dating') && (action!=='Hook up' || person.status!=='dating') && (action!=='Befriend' || !person.friendship) && (action!=='Unfriend' || person.friendship));
}
...
export function interact(life: Life, id: string, action: RelationshipAction, giftId?: string): Life {
  ...
  const accepted=['Hook up','Make love'].includes(action)?romance.accepted:person.strength>=65 && (action!=='Ask out'||romance.compatible);
  const deltas: Record<RelationshipAction,number> = {'Act up':-8,Disrespect:-10,'Suck up':response.delta,Befriend:5,Flirt:3,'Ask for money':given?0:-2,'Ask out':accepted ? 5 : -2,Compliment:response.delta,Conversation:response.delta,Gift:giftDelta,'Hook up':accepted?1+Math.floor(romance.theirEnjoyment/20):-2,'Make love':accepted?1+Math.floor(romance.theirEnjoyment/20):-2,Insult:-12,'Spend time':5,Unfriend:0};
  ...
  const text: Record<RelationshipAction,string> = {
    ...
    Flirt:`I flirted with ${person.name}.`,
  ...
  const happiness = first && ['Hook up','Make love'].includes(action)?accepted?Math.round((romance.yourEnjoyment-40)/10):-2:!first || action==='Ask for money' ? 0 : ['Act up','Disrespect','Insult','Unfriend'].includes(action) || ['Gift','Compliment','Conversation','Suck up'].includes(action) && response.delta<0 ? -3 : ['Ask out','Hook up'].includes(action) && !accepted ? -1 : 2;
