...
export function characters(life: Life): { personal: Person[]; work: Person[]; school: Person[] } {
  ...
  const personal: Contact[] = [...(life.family?.parents.map(parent => ({name:parent.name,relation:parent.relation,strength:parent.relation === 'Mother' ? 86 : 79,group:'Family'})) ?? [{name:`Elena ${surname}`,relation:'Mother',strength:86,group:'Family'},{name:`Daniel ${surname}`,relation:'Father',strength:79,group:'Family'}]),...(life.family?.siblings??[]).map(sibling=>({name:sibling.name,relation:sibling.gender==='Male'?'Brother':'Sister',strength:75,group:'Family'})),...(life.age >= 10 && (!life.family || life.relationships?.['maya-chen']) ? [{name:'Maya Chen',relation:'Best friend',strength:93,group:'Friends'}] : [])];
...
export function interact(life: Life, id: string, action: RelationshipAction, giftId?: string): Life {
  ...
  const happiness = !first || action==='Ask for money' ? 0 : ['Act up','Disrespect','Insult','Unfriend'].includes(action) || ['Gift','Compliment','Conversation','Suck up'].includes(action) && response.delta<0 ? -3 : ['Ask out','Hook up'].includes(action) && !accepted ? -1 : 2;
