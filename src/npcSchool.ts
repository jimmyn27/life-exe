...
export function npcBaseStats(id:string){const seed=[...id].reduce((sum,c)=>sum+c.charCodeAt(0),0);return {Health:75+seed%21,Happiness:65+seed%30,Intelligence:60+seed%36,Appearance:55+seed%40};}
...
export function classmatePopularity(person:SchoolPerson,appearance:number):number{return clamp(appearance+(person.clubs?.length??0)*4+(person.sports?.length??0)*10);}
