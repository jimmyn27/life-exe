...
test('school stages start with Smarts-based grades; studying boosts grades and smarts once yearly',()=>{
 ...
 const before=life(),studied=schoolAction(before,'Study harder');assert.equal(studied.stats.Smarts,62);assert.equal(studied.occupation.school.grades,65);assert.equal(schoolAction(studied,'Study harder'),studied);assert.equal(schoolAction(studied,'Study hard'),studied);
 assert.deepEqual(saved(studied),studied);const next={...studied,age:15,occupation:advanceOccupation(studied,15)};assert.equal(schoolAction(next,'Study harder').occupation.school.grades,70);
});
...
test('positive staff appreciation/agreement improves grades; repeat and low reactions do not add grades',()=>{
 ...
 assert.equal(gain,Math.max(0,Math.ceil(response.delta/2)));if(response.delta>0)positive++;else negative++;
...
test('school actions have stage ordering, dropout age gates, nurse recovery and skip-school effects',()=>{
 ...
 const nursed=schoolAction(before,'Nurse');assert.equal(nursed.stats.Health,75);assert.equal(schoolAction(nursed,'Nurse'),nursed);
 let lostSmarts=false;for(let i=0;i<30;i++){const current=life(12,`skip-${i}`),skipped=schoolAction(current,'Skip school');assert.equal(skipped.stats.Happiness,64);assert.equal(skipped.occupation.school.grades,55);lostSmarts ||= skipped.stats.Smarts<60;}assert.ok(lostSmarts);
...
test('club/sport acceptance raises happiness and rejection lowers it; new clubs save correctly',()=>{
 ...
 for(let i=0;i<25;i++){const before=life(10,`club-${i}`),after=applySchoolActivity(before,'environmental'),accepted=after.occupation.school.activityAttempts.environmental.accepted;assert.equal(after.stats.Happiness,before.stats.Happiness+(accepted?4:-3));if(accepted)yes++;else no++;assert.deepEqual(saved(after),after);}assert.ok(yes>0 && no>0);
});
...
test('friend dance invitations use average friend strength; solo enjoyment can help or hurt happiness',()=>{
 let high=0,low=0,soloGood=0,soloBad=0;for(let i=0;i<30;i++){for(const strength of [100,0]){const current=life(14,`friend-date-${i}`);current.family=generateFamily(current.id,'Smith');const person=characters(current).school[0];current.relationships={[person.id]:{profile:{name:person.name,gender:person.gender,ageOffset:0,education:person.education,occupation:person.occupation},friendship:true,status:'friend',strength,stats:person.stats}};const dance=schoolDance(current,'friends');if(dance.accepted){if(strength===100)high++;else low++;assert.equal(dance.bars.length,2);}else assert.equal(dance.retry,true);}
 const current=life(14,`solo-${i}`),dance=schoolDance(current,'alone');assert.equal(dance.bars.length,1);assert.equal(dance.bars[0].label,'Your Enjoyment');if(dance.life.stats.Happiness>60)soloGood++;if(dance.life.stats.Happiness<60)soloBad++;saved(dance.life);}
 assert.equal(high,30);assert.equal(low,0);assert.ok(soloGood>0 && soloBad>0);
});
