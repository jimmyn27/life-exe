...
const life=(age=14,id='work-profile')=>({id,name:'Sam Smith',firstName:'Sam',lastName:'Smith',city:'New York City',age,birthYear:2000,balance:100,sexuality:'Bisexual',stats:{Health:80,Happiness:70,Intelligence:70,Charisma:70},log:[]});
...
test('yearly Intelligence and Charisma independently move by one, clamp at bounds, and do not reroll during decisions',()=>{
 const combos=new Set();let up=0;for(let i=0;i<200;i++){const old=life(5,`stats-${i}`),next=advanceYear(old);const a=next.stats.Charisma-old.stats.Charisma,b=next.stats.Intelligence-old.stats.Intelligence;assert.ok(Math.abs(a)<=1);assert.ok(Math.abs(b)<=1);for(const key of ['Health','Happiness'])assert.ok(Math.abs(next.stats[key]-old.stats[key])<=1);combos.add(`${a},${b}`);if(a===1)up++;assert.equal(next.occupation.school.grades,next.stats.Intelligence);assert.deepEqual(advanceYear(saved(next)).stats,next.stats);}
 assert.ok(combos.size>=7);assert.ok(up>40 && up<95);
 for(const value of [0,100]){const old=life(4,`bound-${value}`);old.stats.Charisma=value;old.stats.Intelligence=value;const n=advanceYear(old);assert.ok(n.stats.Charisma>=0 && n.stats.Charisma<=100);assert.ok(n.stats.Intelligence>=0 && n.stats.Intelligence<=100);}
});
test('part-time offers unlock by age, have stable wages and ten-to-twenty weekly hours, and reject unavailable jobs',()=>{
 assert.deepEqual(partTimeOffers(life(13)),[]);const young=partTimeOffers(life(14)),older=partTimeOffers(life(16));assert.ok(young.length>=1);assert.ok(older.length>=1);assert.ok(!young.some(j=>j.id==='barista'));assert.ok([...young,...older].every(j=>Number.isInteger(j.hourlyWage)));assert.notDeepEqual(partTimeOffers(life(14)),partTimeOffers({...life(14),age:15}));
...
test('flirt receptiveness is charisma-weighted, changes relationship, and invitations obey age and yearly limits',()=>{
 let negative=false;
 for(let i=0;i<500;i++){let l=life(16,`flirt-${i}`);l.relationships={f:{profile:{name:'Alex Smith',gender:'Female',ageOffset:0,education:'Middle school',occupation:'Student'},strength:99,status:'friend',friendship:true,stats:{Health:80,Happiness:70,Intelligence:70,Charisma:70}}};l.stats.Charisma=100;const p=characters(l).personal.find(p=>p.id==='f');const low=reaction({...l,stats:{...l.stats,Charisma:0}},p,'Flirt'),high=reaction(l,p,'Flirt');assert.ok(high.value-low.value>=59);
 const n=interact(l,'f','Flirt'),r=interactionResult(l,n,p,'Flirt');assert.equal(r.meter.label,'Her receptiveness');assert.ok(n.relationships.f.strength>p.strength);if(r.followUp){const choice=r.followUp.choices[0];assert.equal(choice.relationship.action,'Ask out');const accepted=interact(n,'f',choice.relationship.action,undefined,true);assert.equal(accepted.relationships.f.status,'dating');}
 ...
 const cold={...l,stats:{...l.stats,Charisma:0},relationships:{f:{...l.relationships.f,strength:10}}};const rejected=interact(cold,'f','Flirt');negative ||=rejected.relationships.f.strength<10;
 }
 assert.ok(negative);
});
