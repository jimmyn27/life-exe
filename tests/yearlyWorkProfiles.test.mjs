...
const life=(age=14,id='work-profile')=>({id,name:'Sam Smith',firstName:'Sam',lastName:'Smith',city:'New York City',age,birthYear:2000,balance:100,sexuality:'Bisexual',stats:{Health:80,Happiness:70,Smarts:70,Looks:70},log:[]});
...
test('yearly Smarts and Looks independently move by one, clamp at bounds, and do not reroll during decisions',()=>{
 const combos=new Set();let up=0;for(let i=0;i<200;i++){const old=life(5,`stats-${i}`),next=advanceYear(old);const a=next.stats.Looks-old.stats.Looks,b=next.stats.Smarts-old.stats.Smarts;assert.equal(Math.abs(a),1);assert.equal(Math.abs(b),1);combos.add(`${a},${b}`);if(a===1)up++;assert.equal(next.occupation.school.grades,next.stats.Smarts);assert.deepEqual(advanceYear(saved(next)).stats,next.stats);assert.equal(clearEvents(next).stats.Looks,next.stats.Looks);}
 assert.equal(combos.size,4);assert.ok(up>70 && up<130);
 for(const value of [0,100]){const old=life(4,`bound-${value}`);old.stats.Looks=value;old.stats.Smarts=value;const n=advanceYear(old);assert.ok(n.stats.Looks>=0 && n.stats.Looks<=100);assert.ok(n.stats.Smarts>=0 && n.stats.Smarts<=100);}
});
