...
test('weekly time changes yearly performance around neutral five hours and promotions factor in tenure',()=>{
 for(const hours of [1,5,10]){const current=enrolled(['basketball']);current.occupation.school.activityDetails.basketball={...newMembership(14),hours,performance:70};const next=advanceYear(current);assert.equal(next.occupation.school.activityDetails.basketball.performance,72+(hours-5)*2);assert.equal(next.occupation.school.activityDetails.basketball.years,1);saved(next);}
