...
const life=id=>({id,name:'Sam Smith',city:'New York City',age:18,birthYear:2000,balance:1000,stats:{Health:90,Happiness:80,Intelligence:70,Charisma:60},log:[]});
...
test('gift result reflects its actual effect; repeat outcomes clearly report no additional gain',()=>{
 ...
 const repeated=interactionResult(after,repeat,currentPerson,'Gift','flowers');assert.equal(repeated.change,0);assert.equal(repeated.meter,undefined);assert.match(repeated.text,/already|wait|occasion|gesture/i);assert.doesNotMatch(repeated.text,/\\b(they|them|their)\\b/i);
});
