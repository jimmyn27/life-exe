...
test('dating begins at ten, blocks minor/adult pairing, and intimacy is adult-only with partner action replacement',()=>{
 ...
 const dating={...accepted.l,relationships:{f:{...accepted.l.relationships.f,status:'dating'}}};const p=characters(dating).personal.find(p=>p.id==='f');assert.equal(p.status,'dating');assert.ok(actionUnavailable(dating,p,'Hook up'));assert.equal(actionUnavailable(dating,p,'Make love'),null);assert.deepEqual(saved(interact(dating,'f','Make love')),interact(dating,'f','Make love'));
});
