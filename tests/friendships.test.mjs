...
const life=(age=14,id='friendship')=>({id,name:'Sam Smith',firstName:'Sam',lastName:'Smith',city:'New York City',age,birthYear:2000,balance:100,sexuality:'Bisexual',family:generateFamily(id,'Smith'),stats:{Health:80,Happiness:70,Smarts:70,Looks:70},log:[]});
const saved=l=>parseStore(JSON.stringify(upsertLife(emptyStore(),l))).lives[0];
const friend=(name,strength)=>({profile:{name,gender:'Female',ageOffset:0,education:'Middle school',occupation:'Student'},strength,status:'friend',friendship:true,stats:{Health:80,Happiness:70,Smarts:70,Looks:70}});
test('family starts at one hundred; looks bias is bounded and contact values persist after looks change',()=>{
 ...
 const enrolled={...l,occupation:getOccupation(l)};const peers=characters(enrolled).school.filter(p=>p.relation==='Classmate');assert.ok(peers.every(p=>p.strength>=25 && p.strength<=75));assert.deepEqual(characters({...enrolled,stats:{...l.stats,Looks:0}}).school.map(p=>p.strength),characters(enrolled).school.map(p=>p.strength));
...
test('dating begins at ten, blocks minor/adult pairing, and intimacy is adult-only with partner action replacement',()=>{
 ...
 let accepted;for(let i=0;i<100;i++){let l={...life(18,`adult-${i}`),relationships:{f:friend('Friend',90)}};let p=characters(l).personal.find(p=>p.id==='f');l.family.gender=p.gender==='Female'?'Male':'Female';p=characters(l).personal.find(p=>p.id==='f');const after=interact(l,'f','Have fun');if(after.log.at(-1)?.text.startsWith('I had fun')){accepted={l,p,after};break;}}
 assert.ok(accepted);const result=interactionResult(accepted.l,accepted.after,accepted.p,'Have fun');assert.deepEqual(result.bars.map(b=>b.label),['Your Enjoyment','Her Enjoyment']);assert.deepEqual(saved(accepted.after),accepted.after);
 const dating={...accepted.l,relationships:{f:{...accepted.l.relationships.f,status:'dating'}}};const p=characters(dating).personal.find(p=>p.id==='f');assert.equal(p.status,'dating');assert.equal(actionUnavailable(dating,p,'Have fun'),null);assert.equal(actionUnavailable(dating,p,'Make love'),null);assert.deepEqual(saved(interact(dating,'f','Make love')),interact(dating,'f','Make love'));
});
