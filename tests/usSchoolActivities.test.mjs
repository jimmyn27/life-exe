...
const life=age=>({id:'us-school',name:'Sam Smith',city:'New York City',age,birthYear:2000,balance:100,stats:{Health:90,Happiness:80,Intelligence:70,Charisma:60},log:[]});
test('US schooling follows four-year stages and normalizes existing school saves',()=>{
 ...
 const upgraded=getOccupation(legacy);assert.equal(upgraded.school.level,'Secondary school');assert.equal(upgraded.school.grades,legacy.stats.Intelligence);assert.equal(upgraded.school.startAge,14);assert.deepEqual(upgraded.school.memberships,[]);
});
test('selected clubs and sports are eligible at ten, save correctly, and support gendered baseball labels',()=>{
 assert.equal(schoolActivities.filter(a=>a.group==='Sports').length,12);assert.equal(new Set(schoolActivities.map(a=>a.id)).size,schoolActivities.length);
 for(const activity of schoolActivities){if(activity.group==='Clubs' && activity.name!=='Student Council')assert.ok(activity.name.endsWith(' club'));if(activity.group==='Sports')assert.ok(activity.name.endsWith(' team'));}
